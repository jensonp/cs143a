...
This chapter is not trying to help you survive by memorizing isolated vocabulary such as *kernel*, *interrupt*, *process*, or *cache*. It is trying to build a stable mental model of what the operating system is for, what objects it must manage, and what invariants make the machine usable when many computations share one hardware platform. The central question running through the chapter is always the same: **what mechanism lets the system preserve a global rule at this point?** If you keep that question in view, later chapters on processes, memory, files, synchronization, and scheduling will feel like refinements of a few foundational ideas rather than a pile of unrelated topics.
...
This file is organized to support that skill directly: each module names a **Problem**, explains the **Mechanism**, states the **Invariants**, shows **What Breaks If This Fails**, and gives a **Trace** you can rehearse. The point is not to collect definitions mechanically. The point is to learn how to move from a mechanism to its invariant, from an invariant to its failure mode, and from a failure mode back to the reason the mechanism had to be introduced in the first place. If you can reproduce the traces from memory and explain why each step exists, later chapters feel like added detail instead of new mysteries.

Later chapters should deepen these mechanisms, not rescue undefined language here. The next section supplies the mental models that make the rest of the chapter coherent: the OS as a control system, the kernel as trusted authority, scarcity as the source of concurrency pressure, copies as a source of correctness problems, and scaling as a change in failure shape rather than a simple increase in throughput.
